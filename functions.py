...
def check_eight_hours_feasibility_greedy(w,nurse,check_job,check_day):
    # check for nurse i and day d
    # return True if feasible
    # the following definitions may change
    if not w[nurse,check_day]:
...
# function needed for assignment:
def assign_to_other_nurses_greedy(search_previous,number_nurses,nurses,w_init,z_init,j,d):
    ...
    if len(nurses)>1:
        if len(nurses)<search_previous:
            for cand in nurses:
                if cand!=number_nurses:
                    # if this job fits the schedule of a nurse, choose her as a candidate for this job
                    feasible = check_eight_hours_feasibility_greedy(w_init,cand,j,d) and check_five_days_feasibility(w_init,cand,d)
                    if feasible:
                        tw_feasible=True
                        for tw_exist in w_init[cand,d]:
                            # if there is an overlap in candidate nurse's schedule on day d, skip to next nurse
...
def generate_initial_solution(search_previous):
    ...
    for j in jobs:
        for d in days:
            if jobs[j][d] == 1:
                z_init[j,d] = None
    w_init = find_w_greedy(nurses,z_init)
            ...
            if jobs[j][d] == 1:
                ...
                if feasible:
                    ...
                    if w_init[number_nurses,d]: 
                        ...
                        feasible = check_eight_hours_feasibility_greedy(w_init,number_nurses,j,d)
                        if feasible:
                            tw_feasible=True
                            ...
                            # if there is an overlap, assign job to the other nurses:
                                ...
                                number_nurses,nurses,w_init,z_init,j,d = assign_to_other_nurses_greedy(search_previous,number_nurses,nurses,w_init,z_init,j,d)
...
def generate_initial_solution_SA(search_previous):
    # generate initial solution
    w = {}
    sol = {}
    # Use the following heuristic to generate initial solution with fewer number of nurses:
    ...
    if check_all_jobs_assigned(nurses, sol):
        ...
        generate_initial_solution_SA(search_previous)
...
def first_scenario_greedy(sol, cand_i, cand_j, cand_d):
    # find another nurse to do job cand_j on day cand_d
    # cand_i should undertake this nurse's one job
...
def second_scenario_greedy(sol, nurses, cand_i, cand_j_list, cand_d_list):
    new_i_list = []
    need_nurse = False
    for index in range(len(cand_j_list)):
        cand_j = cand_j_list[index]
...
def generate_neighbour_greedy(sol, nurses):
    global prob1
    global prob2
    # TWO SCENARIOS (MY HYPOTHESIS...), for now let's skip the second one (a bit complex...)
    cand_i = None
    ...
    if random.uniform(0, 1) > prob1:
        ...
        new_i, new_j, new_d = None, None, None
        # choose another nurse, do feasibility check::
        new_i, new_j, new_d = first_scenario_greedy(sol, cand_i, cand_j, cand_d)
    ...
    # 2) INCREASE/DECREASE THE NUMBER OF NURSES BY ONE
        ...
        if random.uniform(0, 1) > prob2:
            ...
            # choose another nurse, do feasibility check:
            new_i_list, nurses = second_scenario_greedy(sol, nurses, cand_i, cand_j_list, cand_d_list)
    ...
    sol['w'] = find_w_greedy(nurses, sol['z'])
...
def greedy_algorithm(sol,nurses,time_limit):    
    ...
    new_sol = generate_neighbour_greedy(sol,nurses)
...
start = timer()
sol,nurses = generate_initial_solution_SA(search_previous)
print("Number of nurses found by initial solution SA : ", len(nurses))
obj = calculate_obj(sol)
print("Objective value for ", objective, "number of nurses seen: " , obj)
