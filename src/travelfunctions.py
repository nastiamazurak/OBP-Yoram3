...
def traveling_times(loc1_x, loc1_y, loc2_x, loc2_y):
    return 2*math.sqrt((loc1_x - loc2_x)**2 + (loc1_y - loc2_y)**2)
                        ...
                        if w_init[number_nurses,d]: 
                            ...
                            if feasible:
                                ...
                                for tw_exist in w_init[number_nurses,d]:
                                    # check overlaps:
                                    #nurse = number_nurses
                                    job2 = find_job_id(z_init, number_nurses, d)
                                    tw_exist_travel_time = tw_exist
                                    tw_exist_travel_time[1] += traveling_times(jobs[j]['x_coord'], jobs[j]['y_coord'], jobs[job2]['x_coord'], jobs[job2]['y_coord'] )
                                    feasible = check_overlap(tw_new,tw_exist_travel_time)
                                    if not feasible: 
                                        tw_feasible=False
                                        break
                                ...
                                if tw_feasible:
                                    print('newwwwwwww')
                                    print(w_init[number_nurses, d])
                                    z_init[j,d] = number_nurses
                                    tw_exist[1] += traveling_times(jobs[j]['x_coord'], jobs[j]['y_coord'],jobs[job2]['x_coord'], jobs[job2]['y_coord'])
                                    w_init[number_nurses,d].append(tw_new)
                                    print(traveling_times(jobs[j]['x_coord'], jobs[j]['y_coord'],jobs[job2]['x_coord'], jobs[job2]['y_coord']))
                                    print(w_init[number_nurses, d])
                                    #I think sometimes it is added twice now?/ to all time windows in w_init instead of only the last
