...
from .helpers import parse_dataset
...
jobs, sets = parse_dataset("uploads/hhc_job_data")
...
def check_final_feasibility(nurses, sol):
    ...
    if (
        no_overlap
        and check_all_jobs_assigned(nurses, sol)
        and eight_hours
        and five_days
    ):
        return True
            ...
            for cand in nurses:
                if cand != number_nurses:
                    ...
                    feasible = check_eight_hours_feasibility(
                        w_init, cand, j, d
                    ) and check_five_days_feasibility(w_init, cand, d)
                    if feasible:
                        tw_feasible = True
